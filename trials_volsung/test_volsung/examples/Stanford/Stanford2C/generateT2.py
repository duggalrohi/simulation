...
Create a TOUGH2 model of Stanford Test Problem #2C
...
r = rocktype(name = 'rock0', permeability = [0.01e-12, 0.01e-12, 0.01e-12], specific_heat = 1000, density = 2650, porosity = 0.2)
r.conductivity = 0
r.dry_conductivity = r.conductivity
r.capillarity = {'parameters' : [0.0] * 7, 'type' : 8}
r.relative_permeability = {'parameters' : [0.30, 0.05, 0, 0, 0, 0, 0], 'type' : 3} # Corey
#r.relative_permeability = {'parameters' : [0, 0, 1, 1, 0, 0, 0], 'type' : 1} # Lin
...
data.parameter['const_timestep'] = 1000
...
# set the print times
data.output_times['time'] = [1,86400]
data.output_times['num_times_specified'] = len(data.output_times['time'])


# set the initial conditions
for i in range(N):
    data.incon[grid.blocklist[i].name] = [0, [90e5, 300,0, 0, 0]]
